...
def read_all_data():
    ...
    path = "/home/qian/Desktop/hw/"
    train_data_path = os.path.join(path, 'SVM/data/Mnist-image/train/0')
    ...
    for c in cName:
        print(c)
        train_data_path = os.path.join(path, 'SVM/data/Mnist-image/train/') + c
...
# create model
def create_svm(dataMat, dataLabel,path,decision):
    clf = svm.SVC(kernel='poly',decision_function_shape=decision)
...
if __name__ == '__main__':
    ...
    # 这里dataMat是每张图片摊开成1*400的格式，下面将进行降维
    #dataMat_pca = PCA(dataMat,50)


    path = '/home/qian/Desktop/hw/SVM/'
    model_path=os.path.join(path,'model/poly_ovr.model')
